...
if uv run --extra dev python -m detect_secrets --version >/dev/null 2>&1; then
  ...
  detect_secrets_exclude='(^|/)(node_modules|\.venv|dist|\.git|coverage|htmlcov)/|pnpm-lock\.yaml|(^|/)\.env$|(^|/)\.env\.(?!example$)[^/]+$|(^|/)ekologus-access-pack-[^/]+/|(^|/)credentials/|(^|/)docs/agents/reports/benchmark/'
