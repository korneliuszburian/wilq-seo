...
for skill_script in .agents/skills/wilq-*/scripts/smoke_skill_contract.py; do
  if [ "$skill_script" = ".agents/skills/wilq-content-operator/scripts/smoke_skill_contract.py" ]; then
    timeout 90s uv run python "$skill_script" --api-base "$skill_api_base" --allow-empty >/dev/null
  else
