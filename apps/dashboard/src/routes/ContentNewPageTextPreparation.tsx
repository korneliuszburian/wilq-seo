...
export function ContentNewPageTextPreparation({
  ...
  if (workspace.error || !workspace.data || !readiness) {
    return <p className="mt-4 rounded-xl border border-wait/30 bg-wait/5 px-3 py-2 text-sm leading-6 text-ink">Nie udało się odczytać danych do przygotowania tekstu. Brief i wybrana wiedza pozostają zapisane; odśwież widok przed kolejnym krokiem.</p>;
  }
  ...
  if (!readiness.planning_input_digest) {
    return <div className="mt-4 rounded-xl border border-wait/30 bg-wait/5 p-3 text-sm leading-6 text-ink"><p className="font-semibold">Nie można jeszcze przygotować tekstu</p><p className="mt-1">Brakuje dokładnych danych roboczych. Odśwież brief przed kolejnym krokiem.</p></div>;
  }
