...
describe("formatContentMetricValue", () => {
  it("keeps content route contract wording sourced from API summaries", () => {
    ...
    expect(routeSource).toContain("refresh.connector_label");
    expect(routeSource).toContain("decision.source_connector_labels");
    ...
    expect(routeSource).toContain("ActionPreviewCard");
    ...
    expect(routeSource).toContain('value={item ? item.recommended_mode_label : "nie pisz"}');
    expect(routeSource).toContain('value={item ? item.status_label : "bramka niedostępna"}');
