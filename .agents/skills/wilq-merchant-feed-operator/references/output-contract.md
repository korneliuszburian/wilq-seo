...
Fetch `POST /api/codex/context-pack` with `{"skill":"wilq-merchant-feed-operator"}` before producing marketing analysis. Use `GET /api/connectors/{connector}/status` for each required connector when readiness matters.
