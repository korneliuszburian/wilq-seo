...
Oczekiwany wynik: podsumowanie feed issues i produktowe akcje do sprawdzenia z Merchant evidence IDs oraz blokady sprawdzenia.
...
8. `Sprawdzenie w WILQ`: pokaż różnicę między statusem akcji w context-packu a bieżącym wynikiem `POST /api/actions/{action_id}/validate`. `valid=true` oznacza tylko ścieżkę sprawdzenia/przygotowania, nie zapis zmian.
