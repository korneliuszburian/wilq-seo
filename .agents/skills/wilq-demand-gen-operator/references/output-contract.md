...
Demand Gen migration/readiness planning across Ads and GA4 evidence.

Expected outcome: Demand Gen readiness findings, blockers and validated action candidates.

## Required API Context

Fetch `POST /api/codex/context-pack` with `{"skill":"wilq-demand-gen-operator"}` before producing marketing analysis. Use `GET /api/connectors/{connector}/status` for each required connector when readiness matters.
