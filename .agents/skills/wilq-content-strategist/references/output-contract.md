...
Fetch `POST /api/codex/context-pack` with `{"skill":"wilq-content-strategist"}` before producing marketing analysis. Use `GET /api/connectors/{connector}/status` for each required connector when readiness matters.
