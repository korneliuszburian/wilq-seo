...
Local visibility readiness and Localo/GBP action candidates.

Expected outcome: Local visibility blockers and safe next actions with explicit missing evidence when Localo access is incomplete.

## Required API Context

Fetch `POST /api/codex/context-pack` with `{"skill":"wilq-localo-operator"}` before producing marketing analysis. Use `GET /api/connectors/{connector}/status` for each required connector when readiness matters.
