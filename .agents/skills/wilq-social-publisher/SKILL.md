...
description: Prepare social publishing action candidates through WILQ API evidence and validation. Use for LinkedIn or Facebook post planning, campaign-to-social adaptation, review-safe social drafts, and social publishing readiness. Must not post or draft unsupported claims without WILQ evidence.
---
...
2. Run `python .agents/skills/wilq-social-publisher/scripts/smoke_skill_contract.py --api-base http://127.0.0.1:8000` when validating the skill/API path.
3. Call `POST /api/codex/context-pack` with `{"skill":"wilq-social-publisher"}` before summarizing metrics, opportunities or action candidates.
...
Goal 001 stub: LinkedIn/Facebook credentials are currently missing, so output must surface blockers rather than pretending publishing is available.
