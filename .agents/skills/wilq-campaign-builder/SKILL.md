...
description: Build campaign action candidates through WILQ API contracts. Use for campaign structures, ad group plans, keyword/asset ideas, payload previews, or review-safe campaign changes. Must validate ActionObjects before any apply path and must not bypass audit requirements.
---
...
2. Run `python .agents/skills/wilq-campaign-builder/scripts/smoke_skill_contract.py --api-base http://127.0.0.1:8000` when validating the skill/API path.
3. Call `POST /api/codex/context-pack` with `{"skill":"wilq-campaign-builder"}` before summarizing metrics, opportunities or action candidates.
...
Goal 001 stub: write actions stay disabled unless WILQ API exposes validated apply support for the action type.
