...
description: Propose Google Ads custom segment candidates only from WILQ API evidence such as search terms, Keyword Planner evidence, existing campaigns and validated source connectors. Use for custom audience or custom segment ideation. Must never invent audience terms.
---
...
2. Run `python .agents/skills/wilq-custom-segments/scripts/smoke_skill_contract.py --api-base http://127.0.0.1:8000` when validating the skill/API path.
3. Call `POST /api/codex/context-pack` with `{"skill":"wilq-custom-segments"}` before summarizing metrics, opportunities or action candidates.
...
Goal 001 stub: do not generate audience terms until WILQ API exposes source term evidence for this workflow.
