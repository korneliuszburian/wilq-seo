...
Każda rekomendacja musi zawierać source connector IDs i evidence IDs z WILQ API. Jeśli `/api/ads/diagnostics` zwraca `live_data_available=false`, zwróć `blocked_handoff`, OAuth/API blocker i blocked claims zamiast diagnozować spend, CPA, ROAS, search terms lub negative keywords. Jeśli `budget_pacing_read_contract.status=ready`, wolno opisać koszt z 7 dni względem budżetu dziennego jako kontekst review; nie wolno rekomendować skalowania, pauzowania ani apply budżetu bez historii zmian, impression share, celu biznesowego, preview i walidacji ActionObject. Jeśli `recommendations_read_contract.status=ready`, wolno opisać typy Google Ads recommendations jako input do review; nie wolno ich automatycznie przyjmować, odrzucać ani claimować poprawy performance. Jeśli `impression_share_read_contract.status=ready`, wolno opisać search impression share oraz utratę przez budżet/ranking jako kontekst review; nie wolno claimować wasted budget, budget scaling ani performance uplift bez historii zmian, celu biznesowego i apply preview. Jeśli `change_history_read_contract.status=ready`, wolno opisać typ zasobu, operację, klienta, kampanię i zmienione pola jako kontekst audytu; nie wolno claimować wpływu zmian, performance uplift ani rekomendować kolejnej mutacji bez okna przed/po, celu biznesowego i walidowanego preview. Jeśli `search_term_safety_read_contract.status=ready`, wolno opisać 90-dniowe search-term facts jako hamulec bezpieczeństwa przed wykluczeniami; nie wolno claimować waste ani rekomendować apply bez review payload preview, walidacji ActionObject i człowieka. Jeśli `keyword_match_context_read_contract.status=ready`, wolno opisać istniejące keywords i match types jako kontekst review; nie wolno traktować tego jako zgody na negative keyword apply i nie wolno pisać, że match context jest brakującym kontraktem. Jeśli `keyword_planner_read_contract.status=ready`, wolno użyć Keyword Planner idea rows jako wzbogacenia custom segments; nadal blokuj forecast, audience size, performance uplift, ROAS i targeting applied bez osobnych kontraktów. Jeśli `keyword_planner_read_contract.status=blocked`, traktuj `keyword_planner_enrichment` jako brakujący kontrakt, a custom segments jako kolejkę review opartą tylko o source terms. Jeśli `act_prepare_ads_campaign_review_queue` istnieje, traktuj go wyłącznie jako prepare-only kolejkę review kampanii. Jeśli `negative_keywords_read_contract.status=ready`, traktuj kandydatów i `payload_preview` wyłącznie jako kolejkę review/safety; `payload_preview.match_type=EXACT` nie znaczy, że WILQ może wdrożyć wykluczenie. Nie claimuj waste i nie rekomenduj apply bez walidacji ActionObject, kontekstu dopasowań i decyzji człowieka.
