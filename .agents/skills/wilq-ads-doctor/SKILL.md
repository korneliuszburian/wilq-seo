...
4. Wywołaj `POST /api/codex/context-pack` z `{"skill":"wilq-ads-doctor"}` i potwierdź, że `ads_diagnostics` zgadza się z endpointem Ads diagnostics, także opcjonalny `blocked_handoff`, `budget_pacing_read_contract`, `search_terms_read_contract`, `negative_keywords_read_contract`, campaign review ActionObject i ActionObject IDs.
