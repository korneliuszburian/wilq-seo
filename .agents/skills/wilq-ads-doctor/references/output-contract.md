...
Diagnostyka Google Ads, jakość kampanii, search terms, negative keywords i bezpieczne akcje do sprawdzenia Ads.

Oczekiwany wynik: ustalenia Ads oparte na evidence, z akcjami do sprawdzenia pozostającymi do sprawdzenia przez WILQ API.

WILQ API pozostaje kanoniczne dla evidence IDs, opportunity IDs, sprawdzenia akcji i audytu. Zewnętrzne narzędzia lub MCP adaptery mogą być tylko źródłem evidence zapisanym w WILQ po zapisaniu do WILQ kontraktów.
...
3. `Diagnoza`: co wspiera `/api/ads/diagnostics`, z niepewnością, jeśli evidence jest zagregowane, stare, niepełne albo zablokowane przez OAuth. Używaj `allowed_metrics`, `missing_read_contracts` i `blocked_claims` z kontraktów API zamiast własnych reguł w opisie.
