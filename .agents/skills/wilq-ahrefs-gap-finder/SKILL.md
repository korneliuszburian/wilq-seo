...
description: Inspect Ahrefs evidence through WILQ API and identify defensible SEO/backlink/content gap opportunities. Use for competitor gaps, domain rating context, backlinks, keyword gaps, and content expansion ideas. Must cite WILQ evidence IDs and must not invent Ahrefs values.
---
...
2. Run `python .agents/skills/wilq-ahrefs-gap-finder/scripts/smoke_skill_contract.py --api-base http://127.0.0.1:8000` when validating the skill/API path.
3. Call `POST /api/codex/context-pack` with `{"skill":"wilq-ahrefs-gap-finder"}` before summarizing metrics, opportunities or action candidates.
...
Goal 001 stub: current Ahrefs adapter persists aggregate Site Explorer domain rating/rank metadata only.
