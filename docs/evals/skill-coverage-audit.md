...
| --- | --- | ---: | --- | --- | --- |
| `wilq-daily-command` | `.local-lab/evals/codex-skill/20260624T020437Z/wilq-daily-command/result.json` | 5 | ready / decision-quality checked | Cross-surface daily loop uses `/command-center`, `daily_decisions`, Merchant, Content, GA4 and Ads; validates the four core daily ActionObjects and omits Localo only because it is outside canonical `daily_decisions`, not because Localo evidence is absent. | Keep Localo/social out of daily action candidates unless canonical daily view includes them or the user explicitly asks for that route. |
...
| `wilq-content-strategist` | `.local-lab/evals/codex-skill/20260624T103515Z/wilq-content-strategist/result.json` | 5 | ready / brief-quality checked | Uses `content_diagnostics.decision_queue`, freshness, BDO/Zielony Ład decisions and deterministic smoke proof for `content_brief_preview_v1`, angle, audience, H1, H2, FAQ, source facts, missing evidence and forbidden claims. | Still review-only: no publish-ready final copy or WordPress write without inventory/canonical checks, review and audit. |
