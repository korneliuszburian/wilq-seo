...
| `wilq-content-operator` | `.local-lab/evals/codex-skill/20260702T043747Z/wilq-content-operator/result.json` | 4 | blocked correctly / UAT-prep | Handles refresh-first content workflow blockers and now requires Service Profile review recorder, promotion preview, `review_requirements`, `source_trace_clear`, `blocked_claims_reviewed` and `follow_up_beads` in actionable output. | Full Wilku content UAT still needs owner session or explicit defer with residual risk. |
