...
build przy concurrent cold miss. Read-only live refresh na tym samym PID
...
a policy, summary, target interpretation i metric tiles są już w typed ownerze.
Nie powtarzać tych projection seamów; legacy helper fasady został usunięty.
Następny krok to parity/runtime proof, nie kolejny duplikat.
...
`vendor_write_possible_count=0`. Zmiana została świadomie zacommitowana i
wypchnięta na `origin/main` jako `3eb9fc9e`; handoff jest w
`docs/handoffs/2026-07-13-kgvy-ads-business-context.md`.
