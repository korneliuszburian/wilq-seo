...
1. `wilq-ads-doctor` current API proof, 2026-06-23.
...
2. Merchant diagnostics decision contract, 2026-06-23.
...
3. Ads Doctor drilldown/API copy cleanup, 2026-06-23.
...
4. Command Center and Ads first-flow copy cleanup, 2026-06-23.
...
5. Ads summary/performance path, 2026-06-23.
   `/ads-doctor` uses `/api/ads/diagnostics?view=summary`; summary reads are
   scoped to the latest completed Google Ads `vendor_read` evidence IDs.
   `/api/actions` uses the same latest Google Ads evidence for Ads
   metric-seeded ActionObjects.
...
1. Run the next high-value Codex skill eval against current API contracts and
   ...
   level read, or `wilq-ga4-analyst` if the next demo focuses on analytics.
