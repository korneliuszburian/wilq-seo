...
- Kontekst journey `/content-workflow` pokazuje teraz pierwsze dokładne fakty GA4 wybranej strony obok GSC (nazwa metryki, wartość, `source_medium`); shared schema przenosi tę informację do browser contractu bez pełnego body WordPress. Dashboard focused suite: 37 testów, typecheck: PASS.
- Checker fixed point wskazał, że prosty `slice(0, 4)` mógł pokazać cztery kanały tej samej metryki. Naprawiono to przez jawny priorytet nazw GA4 i grupowanie kanałów w pojedynczej linii dla każdej metryki; nie sumujemy kanałów bez potwierdzonej semantyki.
- Drugi checker dla `d6116fd4` potwierdził brak krytycznego problemu, ale wymusił trzy LOW doprecyzowania. Helper ma teraz test odwróconej kolejności, deduplikuje identyczne pary kanał/wartość i dopisuje nieznane nazwy metryk po priorytetach w kolejności alfabetycznej.
- Follow-up checker wykrył, że limit czterech grup mógłby po cichu wyciąć nieznaną metrykę. Usunięto ten limit: browser pokazuje wszystkie grupy z bounded 12-fact payloadu; test obejmuje duplicate i `new_metric`.
- Dynamiczne mapowanie źródła: jeśli WordPress dostarcza prawdziwe H2/H3/ACF, są używane w briefie; jeśli materiał ma tylko dostępny `the_content` bez strukturalnych nagłówków, mapa pokazuje `Treść główna (the_content)` zamiast hardkodowanych pytań. Falsifier dla tego przypadku przechodzi.
- Checker `the-content-section-map-v3` jest schema-valid, bez findings; jego luka dowodowa dla gałęzi z prawdziwymi nagłówkami została domknięta drugim focused testem. Obie gałęzie mapowania są teraz sprawdzane.
- Outline draftu rozpoznaje teraz `Treść główna (the_content)` jako istniejący materiał do zachowania/uzupełnienia/przepisania, zamiast generować generyczne purpose. Focused falsifier, Ruff i mypy przechodzą.
- Baseline planning proposal używa tego samego source-aware mechanizmu: `the_content` dostaje purpose „zachowaj/uzupełnij/przepisz” oraz pytanie o istniejącą treść, a strukturalne nagłówki zachowują własne purpose. Commit `50800129` został wypchnięty na `origin/main`. Checker v2 jest schema-valid z trzema LOW evidence gaps (brak pełnych excerptów w briefie), bez potwierdzonego defektu produktu; pierwszy checker został odrzucony transportowo za cytację >20 linii i nie jest PASS.
- Dynamiczny filtr inventory odrzuca teraz zdaniowe lub ponadnormatywnie długie pseudo-nagłówki WordPressa. Na realnym wybranym inventory `content_work_item_inventory_0454c020b0ddbad0062b3d08` poprzednie zdanie prawne przeszło do `Treść główna (the_content)` zamiast stać się wymyśloną sekcją. Falsifier: 3 testy, Ruff, mypy, diff-check i inventory-bound browser proof PASS. Commit `70c889c9` jest na `origin/main`. Bounded checker jest schema-valid z evidence gaps (bez potwierdzonego defektu); disposition: `/home/krn/second-opinion-review/wilq-seo/check/2026-07-20-sentence-like-heading-filter-g0daG4/disposition.md`.
- Body-only WordPress inventory nie jest już błędnie oznaczane jako `missing`: jeśli dokładny rekord ma dostępny `the_content`, planowanie ma `inventory.status=available`, a pusta mapa H2/H3 pozostaje legalna i zachowuje pełny body source. Falsifier: 25 testów planowania, Ruff, mypy, diff-check PASS; live selected snapshot nadal zwraca `plan_allowed` z niezależnymi blockerami kolejnych etapów. Commit `eb47509f` jest na `origin/main`. Bounded checker schema-valid, wyłącznie LOW evidence gaps, bez potwierdzonego defektu; disposition: `/home/krn/second-opinion-review/wilq-seo/check/2026-07-20-the-content-only-planning-oG2XY3/disposition.md`.
- Selected `/content-workflow` nie ogranicza już selektora strony do kolejki okazji: pełny, API-owned `inventory.items` jest dołączany jako deduplikowana grupa `Pełny inventory WordPress`, a wybór nadal idzie tym samym callbackiem work-item route. Falsifier: focused Vitest 2/2, dashboard typecheck, live browser proof 1/1 i diff-check PASS. Commit `aaff654f` jest na `origin/main`. Pierwszy checker został odrzucony transportowo za cytację >20 linii; świeży v2 checker jest schema-valid i pozostawił wyłącznie evidence gaps, disposition: `/home/krn/second-opinion-review/wilq-seo/check/2026-07-20-full-inventory-picker-v2-NuUAst/disposition.md`.
- Queue/browser projection używa teraz tego samego source-aware mapowania co planner: ACF headings mają pierwszeństwo, sentence-like pseudo-heading znika, a body-only `the_content` zostaje dostępne bez fałszywej sekcji. Dla realnego inventory `content_work_item_inventory_0454c020b0ddbad0062b3d08` selected queue zwraca 4 prawdziwe nagłówki ACF, a selected snapshot zwraca `section_count=0` i `content_inventory_status=available` dla wariantu body-only. Falsifier: 4 queue tests, Ruff, mypy, diff-check i live Playwright 1/1 PASS. Commit `36bfdc9c` jest na `origin/main`. Pierwszy checker odrzucony transportowo przez cytację >20 linii; świeży v2 schema-valid z LOW evidence gaps, disposition: `/home/krn/second-opinion-review/wilq-seo/check/2026-07-20-queue-pseudo-heading-alignment-v2-v8yRtE/disposition.md`.
- Homepage nie jest już fałszywie blokowany jako produkt tylko dlatego, że jej szeroki temat zawiera słowo „sorbenty”: topic-based product gating działa dla nie-root pages, a URL/CTA nadal pozostają sygnałami produktowymi. Falsifier: 3 testy focused, Ruff, mypy i diff-check PASS. Live arbitrary inventory `content_work_item_inventory_c90528e2454d9f3de03d2394` ma planning workspace, 8 kandydatów usług, `service_selection_confirmed=false`, brief bez blockerów i brak hardkodowania BDO/outsourcing. Commit `6b592959` jest na `origin/main`. Bounded checker schema-valid, bez findings, z czterema evidence gaps zapisanymi w disposition: `/home/krn/coding/krn/second-opinion-review/wilq-seo/check/2026-07-20-homepage-product-guard-rfC91a/disposition.md`.
- Guard ma teraz także regresję odwrotną: root homepage z produktowym CTA bez Merchant/sklep nadal dostaje `missing_product_evidence`, więc poprawka nie zamienia się w ciche przepuszczanie zakupowych claimów. Cztery falsifiers sales-brief, Ruff i diff-check PASS; commit `a7af99dd` jest na `origin/main`. Drugi bounded checker schema-valid, bez findings, z pięcioma evidence gaps w `/home/krn/coding/krn/second-opinion-review/wilq-seo/check/2026-07-20-homepage-product-guard-regression-nkzrPx/disposition.md`.
- Baseline preserve-first nie odblokowuje już edytora tylko dlatego, że ma podglądowe nagłówki. `section_map_current` jest prawdziwe dopiero dla `codex_generated` z `proposal_id` i sekcjami; po scope review live arbitrary inventory pozostaje na `scope`, ma `can_save=false` i akcję „Uruchom generowanie planu”. Falsifier workspace baseline→generated, 19 testów operatora, Ruff, mypy i diff-check PASS. Commits `0c71e92e` i `69f4d02c` są na `origin/main`. Bounded checker v5 schema-valid; jego guardrail observations nie są potwierdzonymi defektami, a trzy evidence gaps są w `/home/krn/coding/krn/second-opinion-review/wilq-seo/check/2026-07-20-baseline-scope-gate-v5-ytFcM4/disposition.md`.
- Initial-draft nie maskuje już blockerów wejścia jako `stale_planning_input`: `wordpress_material_review_required`, `missing_approved_service_fact`, freshness i source blockers przechodzą z kodem, reason, next step i source lineage do publicznego kontraktu draftu. Focused `test_initial_draft_scope.py` 4/4, Ruff, mypy i diff-check PASS. Commit `3eda1364` jest na `origin/main`. Live technical proof wygenerował na arbitralnym inventory proposal `content_planning_proposal_b620c1158f3c40ecb9ff8de6e5d642ea` z 12 sekcjami, ale initial draft uczciwie zatrzymał się na review źródła `the_content`; nie ma revision ani WordPress write. Checker v3 schema-valid bez findings, disposition: `/home/krn/coding/krn/second-opinion-review/wilq-seo/check/2026-07-20-draft-blocker-lineage-v3-AWDcGR/disposition.md`.
- Front page WordPressa nie spada już bez powodu do HTML tylko dlatego, że nie ma sluga: publiczny REST jest odczytywany bounded collection (`per_page=100`) i dopasowywany po kanonicznej ścieżce `/`. Dla ACF-only strony `the_content` może pozostać puste, ale workflow zachowuje `wordpress_rest`, `source_bound` oraz lineage ACF zamiast udawać brak materiału. Falsifier: inventory suite 21/21, Ruff, mypy, diff-check oraz live snapshot `content_work_item_inventory_c90528e2454d9f3de03d2394` pokazują `wordpress_rest`/`source_bound`; nadal uczciwie pozostaje `content_inventory_status=missing`, bo body nie jest dostępne. To mechanizm dynamiczny, bez wyjątku dla homepage ani ręcznego mapowania sekcji.
- Niezależny checker v2 dla tego fixed pointu jest schema-valid i nie znalazł defektu; pozostawił wyłącznie evidence gaps dotyczące transportu cytowanych excerptów oraz human-only semantyki `source_bound` przy pustym body. Disposition: `/home/krn/coding/krn/second-opinion-review/wilq-seo/check/2026-07-20-wordpress-front-page-rest-v2-ovuarW/disposition.md`.
- Planning input przenosi teraz również dynamiczne nazwy pól ACF (`acf_field_names`) obok nagłówków i body. Dzięki temu planner może rozróżnić „ACF ma dane, ale nie ma schematu sekcji” od braku źródła; nie tworzy z samych nazw pól fałszywych nagłówków. Falsifier: `test_dynamic_planning_input_sources.py` 18/18 oraz ACF-only inventory proof 2/2, Ruff, mypy i diff-check PASS.
- Checker dla ACF field lineage jest schema-valid, bez findings; pozostawił cztery evidence gaps transportowe i dwie human-only decyzje dotyczące live population/akceptacji rewizji. Disposition: `/home/krn/second-opinion-review/wilq-seo/check/2026-07-20-acf-field-lineage-iFNto3/checker.disposition.md`.
- Live readback exact outsourcing page (`/oferta/doradztwo-i-outsourcing-ekologiczny/`) potwierdza tę ścieżkę bez hardkodowania: `wordpress_rest`, 3 ACF fields i 5 ACF section headings; `the_content` pozostaje puste, więc ewentualny full draft nadal wymaga source-aware review. BDO page zwraca REST body 3659 znaków oraz 1 ACF field. To dowód odczytu źródła, nie akceptacja treści ani UAT.
- ACF-only page nie jest już pustym wejściem dla planera: connector wyciąga bounded, zsanityzowany tekst z publicznych wartości ACF (pomija linki, media IDs, kolory i techniczne layout keys), oznacza `extraction_region=wordpress_rest.acf`, a istniejące ACF headings nadal sterują mapą sekcji. Live outsourcing zwraca 3044 znaki materiału z 5 headingami; test regresyjny i planowanie przechodzą. To nadal read-only source material — żadnego ACF write ani auto-approval.
- Checker ACF material extraction jest schema-valid bez findings; zapisał evidence gaps dotyczące pełnej listy pól technicznych/media oraz trzech human-only decyzji o boundzie i lineage. Disposition: `/home/krn/coding/krn/second-opinion-review/wilq-seo/check/2026-07-20-acf-material-extraction-MLEEen/disposition.md`.
- Planning generation odzyskuje teraz trwały job `queued/not_started`, którego executor zniknął po reloadzie procesu. Kolejny POST dla tego samego exact digestu ponownie przejmuje job tylko raz; GET pozostaje odczytem. Falsifier publicznego API przechodzi, Ruff/mypy/diff-check PASS. Commit `7e219ac8`.
- Live generated proposal dla outsourcingu: `content_planning_proposal_c03f7663a2044c4f953dec1933fe9982`, `codex_generated`, 5 sekcji i 5/5 inventory mappings. Sekcje mają query terms, evidence IDs, source material IDs i knowledge-card IDs; page assets obejmują title/H1/lead/meta, 3 FAQ, 2 CTA i 1 link. `referencje` dostało `remove_review_required`, więc model nie publikuje niezweryfikowanych referencji. To technical live proof, nie Wilku UAT ani owner acceptance.
- Checker planning recovery jest schema-valid bez findings; pozostawił evidence gaps dotyczące transportu excerptów, GET/dedup oraz dwóch human-only decyzji. Disposition: `/home/krn/coding/krn/second-opinion-review/wilq-seo/check/2026-07-20-planning-recovery-live-proof-dMPZIl/disposition.md`.
- Quality review dla trwałej rewizji v2 nie wymaga już legacy `draft_package`: API hydratyzuje z aktualnego snapshotu claim ledger i Sales Brief, a quality gate analizuje exact revision digest. Live outsourcing zwrócił `needs_changes` bez blockerów (dwa advisory findings: signal review i measurement po publikacji), zamiast fałszywego `missing_draft_package/missing_claim_ledger`. Focused falsifier i Ruff/mypy/diff-check PASS.
- Niezależny fixed-point checker dla v2 quality review jest schema-valid i ma zero findings; disposition: `/home/krn/coding/krn/second-opinion-review/wilq-seo/check/2026-07-20-durable-v2-quality-review-Ht90pq/disposition.md`. Pozostałe evidence gaps dotyczą wyłącznie granic transportu excerptów oraz decyzji human-only; nie są dowodem gotowości do akceptacji ani publikacji.
- Selected quality-review hydratyzuje teraz także pełny `ContentWorkItem` z exact snapshotu. Compact queue candidate z przeglądarki nie może już wyzerować inventory/duplicate/freshness i wywołać fałszywego blokera. Live BDO replay na rewizji `2dc52e5d2d963a7b68dfad649ae496fce07e2d67a6b9cf0c6eb49016e4d3c4df` zwrócił `needs_changes`, 0 blockerów i wyłącznie advisory `sales_brief_signal_review_required`; focused regression 1 passed. Niezależny checker jest przygotowany po otwarciu okna Claude.
- Contract child-revision ogranicza teraz `sections.minItems/maxItems` do liczby jawnie wybranych sekcji. Live BDO Codex turn wcześniej dwukrotnie zakończył się typed `section_scope_mismatch` bez częściowego zapisu; schema guard zmniejsza tę klasę błędu przed finalizacją. Falsifier schema 1 passed, Ruff i diff-check PASS. To nadal nie jest dowód udanej child revision ani semantic approval — kolejny live turn pozostaje jawnie reviewable.
- Child-revision turn dostał dodatkowo dynamiczny count/requirements w application context, jawne polecenie zachowania dokładnej mapy evidence oraz dla pojedynczej sekcji exact min/max `evidence_ids`. Następny live BDO turn zakończył się `runtime_failed` bez rewizji i bez częściowego zapisu; runtime jest teraz osobnym blockerem od scope validation. Focused schema falsifier 1 passed.
- Minimalny smoke app-servera przeszedł (`completed`), a dokładny child request ujawnił nieobsługiwane przez runtime `uniqueItems`; usunięto tę cechę, zachowując min/max. Następnie wykryto rozjazd starego generation contractu z evidence exact v2 revision: preview i quality gate dostają teraz lineage z wybranej rewizji. Live BDO child revision zakończyła się `created`: `content_revision_31fbcc00689b4a2aad98b1f9194a1ef2`, v4, base `content_revision_a76ee0e1fd4841bf9de08e49756320a1`, digest `6a5214c9113b410d5c5fe409587663d6b7ebba4846453f6297a3fd4eff162c1a`, `publish_ready=false`, quality `needs_changes` bez blockerów (weak CTA, internal links, signal review, measurement). To dowód technicznego child-revision, nie semantic approval ani UAT.

### 2026-07-20 — pełny harness przeszedł przez runtime i draft

- Naprawiono fallback inventory: materiał ACF/the_content bez wykrytych nagłówków zachowuje nagłówki z canonical inventory (`9e8c055a`).
- Naprawiono koszt odczytu: planner nie hydratyzuje już całej historii metric-fact evidence przy każdym snapshotcie; inventory czyta exact WordPress refresh evidence, a metryki pozostają własnością demand/measurement projections (`7e1c2fab`).
