...
`/api/content/diagnostics` can still expose Ahrefs decisions for the broader
Content Planner and `wilq-content-strategist`, but
`POST /api/codex/context-pack {"skill":"wilq-gsc-content-doctor"}` now returns
only the GSC/WordPress content decision subset. Guardrails:
...
and `scripts/codex_skill_eval.sh` supports `forbidden_connectors` so the GSC
eval fails if `ahrefs` appears in top-level or recommendation
`source_connectors`. Non-interactive eval passed at
`.local-lab/evals/codex-skill/20260623T150248Z/wilq-gsc-content-doctor/result.json`
with `pl-PL`, `api_used=true`, source connectors
...
Completed at 2026-06-23 17:33 CEST:
`wilq-demand-gen-operator` eval contract now explicitly expects the current
correct state to be blocked/review-only. Non-interactive eval passed at
`.local-lab/evals/codex-skill/20260623T153134Z/wilq-demand-gen-operator/result.json`
with `pl-PL`, `api_used=true`, `blocked=true`, zero recommendations, validated
`act_review_demand_gen_readiness`, `operator_usefulness_score=4` and no safety
findings. The blocker is useful product behavior: WILQ sees Ads/GA4 evidence
...
2026-06-23 Merchant first-flow cleanup: `/merchant` no longer prints raw
`ev_*` evidence IDs or raw `act_review_merchant_feed_issues` in first-flow
Merchant decision cards or the `Bezpieczny tryb pracy` panel. Those cards show
evidence/action counts and marketer copy says `ActionObject review`; lower
`Dowody i ograniczenia Merchant` and ActionObject validation remain the place
for raw traceability. Browser proof with agent-browser on
`http://127.0.0.1:5173/merchant`: raw trace links start only under the lower
...
measurement rows. Titles now include reporting context, for example
`GA4: napraw pomiar - (not set) / (not set)` and
`GA4: napraw pomiar - (not set) / google / organic`; `/ga4` renders the same
distinct labels. Focused proof: RED/GREEN GA4 API title test, existing GA4
diagnostics contract test, Python ruff OK and mypy OK.
...
measurement/operator cards or the `Bezpieczny tryb analityki` panel. Those
cards show evidence/action counts and the validation CTA says `Waliduj review
GA4`; lower `Dowody i ograniczenia GA4`, ActionObject validation and review
preview sections remain the place for raw traceability. Browser proof with
...
contract now says `aktualny odczyt kampanii` while preserving the same blocked
claim boundary for change impact until current campaign readout, change rows
...
the blocked decision blocks only missing contracts for GBP, competitor
visibility and local tasks instead of saying "bez Localo facts". Live proof:
...
local_tasks]`. Focused proof: RED/GREEN Localo partial-visibility diagnostics
test plus access-ready and missing-access Localo diagnostics tests, Python ruff
OK and mypy OK.
...
2026-06-23 Custom Segments follow-up: `/ads-doctor/custom-segments` no longer
prints the debug context-pack endpoint and `skill=wilq-custom-segments` string
...
now shows `Dowody: N ID` instead of printing raw evidence IDs inline and renders
`metric_summary` as a compact `Metryki: key=value` line instead of raw JSON.
This keeps the supporting registry operational without making cards look like
trace-log dumps. Focused proof: connector refresh run card unit test passed,
dashboard typecheck OK and dashboard lint OK.

2026-06-23 Ads value readout follow-up: `/api/ads/diagnostics.operator_summary`
now exposes typed top-line campaign totals: clicks, impressions, cost micros,
conversions and conversion value. `/ads-doctor` renders the same fields in the
first `Aktualny odczyt Ads` section as `Wartości Ads`, before
review-count metadata. This is a marketer value surface, not a profitability
verdict: ROAS, CPA, waste, negative keyword apply and budget/recommendation
apply remain blocked by the existing read/action contracts. Focused proof:
Ads diagnostics API contract RED/GREEN test passed, Ads Doctor route test
passed, Python ruff/mypy passed, dashboard typecheck and lint passed.
...
metric tile labels. Cost and conversion value are shown in account units with
detected currency, not as raw micros or unlabelled money-like numbers. Command
Center therefore shows real Ads volume/value facts on the first screen without
React-side Ads recomputation. Focused proof: Command Center Ads API contract
...
2. **Continue the measured Command Center performance work.**
   ...
   `build_marketing_brief`. The 2026-06-23 action-candidate bottleneck is also
