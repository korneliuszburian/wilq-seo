...
- Dashboard patch/minor dependencies are current where they do not require a
  ...
  `lucide-react`, `jsdom`, `@types/node`, `typescript`, `zod` and `tailwindcss`
  with focused verification. Remaining outdated JS packages are major
  migrations and must be handled as separate verified slices, not mixed into
  marketer-copy or route cleanup.
