...
- Dashboard patch/minor dependencies are current where they do not require a
  ...
  `lucide-react`, `jsdom`, `@types/node`, `typescript`, `zod`, `tailwindcss`,
  `vite`, `vitest` and `@vitejs/plugin-react` with focused verification. JS
  workspace dependencies are current as of 2026-06-29; future dependency work
  should stay separate from marketer-copy or route cleanup.
