...
| `/content-workflow` | 6.5/10 operator workflow; 5/10 real text quality; 8/10 handoff safety | Primary five-step "Treści i SEO" journey with persisted planning decisions and exact revisions. Scope shows compact typed GSC query rows with metrics, period and freshness; the same rows are mapped to sections by exact page/evidence lineage. Noisy multi-`-site:` operator queries are excluded. Ads/Planner stay empty unless term, page and service all match exactly. `wilq-content-operator` now follows this same journey instead of legacy client-owned stages. | Existing queue/browser-safe snapshot and journey reads, exact planning review, revision save/review, exact Codex proposal, activation/readiness/readback and canonical ActionObject endpoints. Demand evidence is part of the planning proposal/digest; there is no extra endpoint or client-side phrase extraction. Local executor readiness is shared with `openai_codex` and uses CLI/login, never an API key. | Save refetches the API-owned step; stale planning review preserves the note and offers explicit refresh. Changed planning/demand invalidates downstream authority. Live proof has 2 fresh GSC rows mapped to 3 sections, 0 guessed Ads/Planner rows, desktop/mobile 1/1 and zero content POST: `.local-lab/proof/dashboard-content-workflow/2026-07-15T23-53-12-663Z/`. Skill smoke preserves the real `scope` blocker, fresh non-interactive eval scores usefulness 9/10, and live Codex runtime status is configured without exposing auth path or content. | Service Profile remains review-required; Ads/Planner facts currently lack exact page+service mapping, real measurement and verified internal links are missing. Real generated text remains 5/10 and the form has not had real Wilku UAT. | Obtain owner-reviewed Service Profile/Wilku UAT and test a review-ready package for one exact service/section. |
