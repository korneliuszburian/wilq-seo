...
layout count, missing review/audit/handoff steps and admin/dev links. Keep the
next slice focused on dev draft apply/readback and ACF section editing, not on
...
| `/content-workflow` | 63% | Strongest content mechanic but still overloaded below the fold. Top now shows one selected content item with public URL/title/H1/sections, GSC reason, blockers, Claim Ledger summary and a consolidated dev draft WordPress panel. Latest screenshot: `/tmp/wilq-content-workflow-dev-draft.png`. | `GET /api/content/work-items/queue`, `GET /api/content/work-items/{id}/snapshot`, `GET /api/content/wordpress/authoring-profile`, draft readiness/activation endpoints in `api.ts` | Step workflow, selected public page, dev draft state, ACF layout count, missing review/audit/handoff steps, dev/admin links, Claim Ledger, draft-only posture. | Lower half is still too long and card-heavy. It does not yet show actual dev draft readback, real ACF row contents or editable proposed section mapping. | Next: execute safe dev draft/readback path, show current dev page sections/ACF rows, then cut or collapse lower legacy panels into one active writing step. |
