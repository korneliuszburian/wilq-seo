...
Next work is evidence-backed candidate density under parent `r564`; mobile UX (`r564.3`) and canonical dev-only apply (`c9h9.4`) are closed. The deleted legacy planner
must not return.
...
| `/content-workflow` | 6/10 | Primary "Treści i SEO" workspace. Live queue is blocked at 1 of 2 actionable candidates (minimum 3), while content freshness is now fresh after read-only refresh. Queue and selected snapshot carry typed freshness; queue-owned first card renders before snapshot/enrichment, API prewarm keeps cold queue under 5 s, and mobile has a compact decision card before heavy detail. | Existing queue, snapshot, enrichment, authoring-profile, activation/readiness and action endpoints in `api.ts`; no new endpoint is needed. Direct execution is dry-run only; React forces `mode=dry_run` and null authorization. | Concrete public page selection; public/dev role split; fresh source status; evidence lineage; queue decision and safe next step before subordinate reads; mobile decision/blocker/safe CTA; no duplicate-create/direct-live CTA; publish/destructive false. | Fresh browser proof now passes layout; live queue remains blocked only by the minimum actionable threshold and must not invent a third topic. | Parent `r564` owns evidence-backed candidate density; preserve the closed `r564.3` viewport and removed live path. |
...
| `/settings` | 75% | Source health center has stale-source read-only refresh buttons, API-owned typed refresh state (`ready`, `stale`, `partial`, `failed`, `blocked`, `unknown`), plus optional async `queued/running` refresh with polling, safe next step and affected decisions. | Existing `GET /api/connectors` with `refresh_state`, `POST /api/connectors/{id}/refresh`, `GET /api/connectors/refresh-runs/{run_id}` | Access/freshness, missing source impact, explicit refresh progress/result and safe next step. | Automatic stale-trigger on page load remains intentionally off; current refresh is operator-triggered read-only and async polling invalidates view-models after completion. | Preserve the typed state and cache invalidation; only add automatic refresh when a real stale decision and rate-limit policy are explicit. Browser proof `.local-lab/proof/4wwo-sources-refresh-state.png` and mobile variant. |
