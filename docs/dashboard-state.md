...
| `/content-workflow` | 73% | Strongest content mechanic but still overloaded below the fold. Top now shows one selected content item with public URL/title/H1/sections, GSC reason, blockers, Claim Ledger summary and a consolidated dev draft WordPress panel. That panel can prepare the ActionObject write authorization, create a gated dev WordPress draft and show the persisted activation-packet readback after reload. Local proof on 2026-07-07 created dev draft ID `1275`; publish/destructive update stayed false. Latest screenshot: `/tmp/wilq-content-workflow-dev-draft-created.png`. | `GET /api/content/work-items/queue`, `GET /api/content/work-items/{id}/snapshot`, `GET /api/content/wordpress/authoring-profile`, draft readiness/activation endpoints in `api.ts`, action preview/review/confirm endpoints, `POST /api/content/work-items/wordpress-draft-execution` | Step workflow, selected public page, dev draft state, ACF layout count, missing review/audit/handoff steps, dev/admin links, Claim Ledger, draft-only posture, ActionObject authorization, gated dev draft creation and persisted dev draft readback. | Lower half is still too long and card-heavy. It still does not show real ACF row contents or an editable proposed section mapping against the dev page. The local dev draft proof used a Codex/local operator smoke review, not Wilku content approval. | Next: read current dev draft content/ACF rows from `ekologus.dev.proudsite.pl`, show editable section/ACF mapping in the top workbench, then cut or collapse lower legacy panels into one active writing step. |
