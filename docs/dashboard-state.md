...
ActionObject authorization step and a gated dev draft creation action. Keep the
next slice focused on actual dev draft readback and ACF section editing, not on
adding more top-level cards.
...
| `/content-workflow` | 67% | Strongest content mechanic but still overloaded below the fold. Top now shows one selected content item with public URL/title/H1/sections, GSC reason, blockers, Claim Ledger summary and a consolidated dev draft WordPress panel. That panel can prepare the ActionObject write authorization and exposes a gated "Utwórz szkic na dev" action when readiness returns a write authorization and the handoff is ready. Latest screenshot: `/tmp/wilq-content-workflow-dev-write-actions.png`. | `GET /api/content/work-items/queue`, `GET /api/content/work-items/{id}/snapshot`, `GET /api/content/wordpress/authoring-profile`, draft readiness/activation endpoints in `api.ts`, action preview/review/confirm endpoints | Step workflow, selected public page, dev draft state, ACF layout count, missing review/audit/handoff steps, dev/admin links, Claim Ledger, draft-only posture, ActionObject authorization and gated dev draft creation path. | Lower half is still too long and card-heavy. It does not yet show actual dev draft readback, real ACF row contents or editable proposed section mapping. The live dev draft creation path is present in UI/tests but still needs a real local click/readback proof before claiming it works end to end. | Next: run the full local dev draft flow against `ekologus.dev.proudsite.pl`, read back the created draft/ACF state, then cut or collapse lower legacy panels into one active writing step. |
