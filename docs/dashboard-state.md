...
Next work is cold-load latency (`c9h9.6`), then canonical dev-only apply
(`c9h9.4`) and mobile UX (`r564.3`). The deleted legacy planner
must not return.
...
| `/content-workflow` | 5/10 | Primary "Treści i SEO" workspace. Live queue is blocked at 0 of 2 actionable candidates (minimum 3). Queue and selected snapshot now carry typed freshness, and stale GSC/public WordPress evidence blocks the homepage with a refresh-first next step. Fresh 1440×900 and 390×844 renders were reviewed on 2026-07-11. | Existing queue, snapshot, enrichment, authoring-profile, activation/readiness and action endpoints in `api.ts`; no new endpoint is needed. Direct execution is dry-run only; React forces `mode=dry_run` and null authorization. | Concrete public page selection; public/dev role split; freshness/source blocker above fold; evidence lineage; preview-only CTA when data is fresh; no duplicate-create/direct-live CTA; publish/destructive false. | Cold selected snapshot exceeds 30 s in Playwright. Full decision/CTA remains below mobile triage until `r564.3`; raw diagnostics still has historical decision rows outside the queue seam. | Implement `c9h9.6`, then `c9h9.4` and `r564.3`. `c9h9.5` is closed; `r564.2` is closed; do not restore its removed live path. |
