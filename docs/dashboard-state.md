...
| `/knowledge` | 55% | Admin/review support surface with current "Wiedza" IA. Operating-map remains the only initial read; cards/playbooks defer until disclosure. Existing map cache is 15 s; cold/warm HTTP after restart `18.940732 s` / `0.053012 s`. First decision/blockers render in the 3 s browser proof without a global loader. | `GET /api/knowledge/cards`, `/api/knowledge/playbooks`, `/api/knowledge/operating-map`; `build_knowledge_operating_map_cached()` and existing disclosure controls | Source lineage and claim review. | Cold operating-map build is still >5 s; prewarm would block startup health, so the gap remains in `c9h9.12`. | Continue API-owned map build reduction/non-blocking strategy; do not re-enable concurrent subordinate reads. |
