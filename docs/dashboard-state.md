...
| `/content-workflow` | 6/10 | Primary "Treści i SEO" workspace. Live queue is blocked at 0 of 2 actionable candidates (minimum 3). Queue and selected snapshot carry typed freshness; stale GSC/public WordPress evidence blocks the homepage with a refresh-first next step. Queue-owned first card now renders before snapshot/enrichment, and API prewarm keeps cold queue under the 5 s budget. Freshness and cold-load renders were reviewed on 2026-07-11. | Existing queue, snapshot, enrichment, authoring-profile, activation/readiness and action endpoints in `api.ts`; no new endpoint is needed. Direct execution is dry-run only; React forces `mode=dry_run` and null authorization. | Concrete public page selection; public/dev role split; freshness/source blocker above fold; evidence lineage; queue decision and safe next step before subordinate reads; preview-only CTA when data is fresh; no duplicate-create/direct-live CTA; publish/destructive false. | Full decision/CTA remains below mobile triage until `r564.3`; raw diagnostics still has historical decision rows outside the queue seam. | Implement `c9h9.4`, then `r564.3`. `c9h9.5` and `c9h9.6` are closed; do not restore the removed live path. |
