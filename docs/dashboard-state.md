...
| `/content-workflow` | 6.5/10 operator workflow; 5/10 real text quality; 8/10 handoff safety | Primary five-step "Treści i SEO" journey with persisted planning decisions and exact revisions. Scope now shows compact typed GSC query rows with metrics, period and freshness; the same rows are mapped to sections by exact page/evidence lineage. Noisy multi-`-site:` operator queries are excluded from planning. Ads/Planner stay empty unless term, page and service all match exactly. | Existing queue/browser-safe snapshot and journey reads, exact planning review, revision save/review, exact Codex proposal, activation/readiness/readback and canonical ActionObject endpoints. Demand evidence is part of the planning proposal/digest; there is no extra endpoint or client-side phrase extraction. | Save refetches the API-owned step; stale planning review preserves the note and offers explicit refresh. Changed planning/demand invalidates downstream authority. Live proof has 2 fresh GSC rows mapped to 3 sections, 0 guessed Ads/Planner rows, desktop/mobile 1/1 and zero content POST: `.local-lab/proof/dashboard-content-workflow/2026-07-15T23-53-12-663Z/`. | Service Profile remains review-required; Ads/Planner facts currently lack exact page+service mapping, real measurement and verified internal links are missing. Real generated text remains 5/10 and the form has not had real Wilku UAT. | Align the content operator skill with this exact planning path under `amj2.4`; separately replace historical `CODEX_API_KEY` readiness and obtain owner-reviewed Service Profile/Wilku UAT. |
