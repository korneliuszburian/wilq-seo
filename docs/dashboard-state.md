...
| `/ads-doctor` | 60% | Existing `GET /api/ads/diagnostics?view=summary` now has 15 s read-through cache; cold/warm HTTP after restart measured `1.426757 s` / `0.016956 s`. Shared schema accepts the API-owned summary and route has a safe Ads-loading shell while actions continue loading. Full proof shows 5 decisions, freshness, evidence and blocked ROAS/revenue claims. | Existing summary view + `build_ads_diagnostics_summary_cached()`; no new endpoint. | Fresh Ads data, campaign rows, decisions, action IDs; no writes or unsupported claims. | Full cold browser still needs a focused current E2E assertion; legacy `dashboard-api.spec.ts` Ads expectations belong to `c9h9.8`. | Finish `c9h9.9` with focused cold Playwright/current IA proof; preserve read-only cache and startup health. |
