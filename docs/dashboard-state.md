...
| `/ahrefs` | 6/10 | SEO signal drilldown with an API-owned manual-only cross-check state. Current live data has 6 candidates, zero exact GSC/WordPress matches and zero queue actions. | `GET /api/ahrefs/diagnostics`, `getAhrefsDiagnostics()` and the shared `ContentAhrefsCandidateRow` schema. | Gap/authority signals, freshness, exact/weak/missing source lineage, and a safe manual next step without raw IDs above candidate details. | The generic data-ready cards still precede the manual cross-check blocker in the first desktop viewport, so a marketer can initially see `gotowe` before the actual decision boundary. Ahrefs cannot drive publication or a content brief alone. | Keep it as input to `/content-workflow`; `3bst.7` should elevate the existing API blocker/safe step above the card gallery without adding React matching rules or a direct action for weak similarity. |
