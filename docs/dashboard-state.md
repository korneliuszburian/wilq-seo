...
| `/settings` | 70% | Source health center has stale-source read-only refresh buttons plus API-owned typed refresh state (`ready`, `stale`, `partial`, `failed`, `blocked`, `unknown`), safe next step and affected decisions. | Existing `GET /api/connectors` with `refresh_state`, `POST /api/connectors/{id}/refresh` | Access/freshness, missing source impact, explicit refresh result and safe next step. | Refresh execution is still synchronous, so queued/running is represented by the mutation pending state; automatic retry remains outside this slice. | Preserve the typed state and cache invalidation; next 4wwo slice can add polling/automatic refresh only when a real stale decision needs it. Browser proof `.local-lab/proof/4wwo-sources-refresh-state.png`. |
