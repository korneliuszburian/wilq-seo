...
| `/content-workflow` | 6/10 operator workflow; 5/10 real text quality; 8/10 handoff safety | Primary five-step "Treści i SEO" journey with persisted planning decisions and exact revisions. API now owns one proposal/digest for service, intent, CTA and section map; append-only scope/section decisions gate the first revision. `dev_draft` readiness comes from the same revision-bound WordPress handoff used by the inline ActionObject. | Existing queue/browser-safe snapshot and journey reads, exact `POST /api/content/work-items/{work_item_id}/planning-review`, revision save/review, exact Codex proposal, activation/readiness/readback and canonical ActionObject endpoints. Normal browser JSON has planning data and headings/readiness, not prompt/model input. | Stale planning review is rejected, retries are idempotent, section map cannot be approved before scope, and a changed proposal reblocks both decisions. Exact human approval can continue into preview/review/confirm/impact without fabricated readiness. | Dashboard forms for the two planning decisions and browser resume/conflict proof are not implemented yet. Service Profile remains review-required; typed Ads/keyword evidence, real measurement and internal links are missing. Real generated text remains 5/10. | Finish the planning review UI/browser slice under `amj2.1`, then add typed demand evidence; separately replace historical `CODEX_API_KEY` connector readiness. |
