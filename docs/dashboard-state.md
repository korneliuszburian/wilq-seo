...
| `/content-workflow` | 6/10 | Primary "Treści i SEO" workspace. Live queue has 2 total candidates, 0 actionable, and requires a minimum of 3, while GSC and public WordPress freshness are stale. Queue and selected snapshot carry typed freshness; selected snapshot also owns a compact per-item Service Profile decision: typed service binding, approval and policy for that card (full Claim Ledger stays separate), source/freshness/evidence, first blocker and safe next step. IDs remain in disclosure; mobile retains a compact decision card before heavy detail and has no horizontal page overflow at 390 px. Ahrefs supporting rows distinguish exact proof from weak manual similarity. Browser proof now branches explicitly: ready queues assert the authoring workbench, blocked queues assert the Polish blocker, safe next step and no overflow. | Existing queue and existing snapshot plus enrichment, authoring-profile, activation/readiness and action endpoints in `api.ts`; no new endpoint. Direct execution is dry-run only; React forces `mode=dry_run` and null authorization. | Concrete public page selection; public/dev role split; fresh source status; service/claim context next to current page; evidence lineage; queue decision and safe next step; no duplicate-create/direct-live CTA; publish/destructive false. | Service Profile is honestly review-blocked; live queue remains blocked and must not invent a third topic. Production-depth and final draft remain unavailable until human review. | `r564.5` and `r564.6` are closed. Preserve the snapshot-owned context and the closed mobile viewport `r564.3`; do not infer service from enrichment or restore a live path. |
