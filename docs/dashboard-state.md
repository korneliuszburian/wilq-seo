...
| `/content-workflow` | 6.5/10 operator workflow; 5/10 real text quality; 8/10 handoff safety | Primary five-step "Treści i SEO" journey with persisted planning decisions and exact revisions. The selected work-item snapshot expands exact current-page GSC planning facts instead of inheriting the tactical queue limit: the homepage currently aggregates 29 signals to 47 impressions and 3 clicks, while the UI shows the top four and explains the coverage. The context card and planning panel use that same snapshot. Query rows remain page-level evidence; a section receives a term only after conservative token relevance to its heading/purpose. Typed `section_mapping_status` distinguishes `lexical_relevance` from `page_only`. Ads/Planner stay empty unless term, page and service all match exactly. `wilq-content-operator` follows this same journey. | Existing queue/browser-safe snapshot and journey reads, exact-page MetricStore facts for the selected snapshot, exact planning review, revision save/review, exact Codex proposal, activation/readiness/readback and canonical ActionObject endpoints. Demand evidence is part of the planning proposal/digest. Local executor readiness uses CLI/login, never an API key. Audit authority is server-owned as `local_operator` with `local_unverified` trust; client labels cannot create expert acceptance. | Save refetches the API-owned step; stale planning review preserves the note and offers explicit refresh. Changed planning/demand invalidates downstream authority. Focused API proof retains all exact current-evidence facts and recomputes the summary; focused UI proof rejects a stale queue reason after snapshot load. Browser proof shows 47/3 in both visible contexts, the top-four-of-29 explanation and no old 17/3 summary. No guessed Ads/Planner rows or all-section evidence inheritance were introduced. Skill smoke preserves the real `scope` blocker, fresh non-interactive eval scores usefulness 9/10, and live Codex runtime status is configured without exposing auth path or content. | Service Profile remains review-required; `local_unverified` is not owner/expert acceptance; Ads/Planner exact mapping, real measurement, verified internal links and Wilku UAT are missing. Real generated text remains 5/10. | Have Wilku mark the current homepage scope `needs_changes` or approve it explicitly; then test a review-ready package for one exact service/section. |
