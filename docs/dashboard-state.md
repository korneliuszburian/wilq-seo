...
| `/content-workflow` | 6/10 operator workflow; 5/10 real text quality; 8/10 handoff safety | Primary five-step "Treści i SEO" journey with persisted exact revisions, reload/resume/conflict and exact review. In `draft`, Wilku can ask Codex to improve selected sections of the exact reviewed base. After exact approval, `dev_draft` readiness now comes from the same revision-bound WordPress handoff used by the inline ActionObject; the API no longer reports a missing seam that already exists. | Existing queue/browser-safe snapshot and journey reads, revision save/review, exact `POST /api/content/work-items/{work_item_id}/draft-revisions/{base_revision_id}/codex-proposal`, activation/readiness/readback, canonical ActionObject endpoints, and local Codex app-server through WILQ API. Normal browser JSON has headings/readiness only, not prompt/model input. | A marketer can move from exact human approval into the inline preview/review/confirm/impact flow without the dashboard fabricating readiness. Missing revision, changed context and mismatched binding remain blocked; publish/update/delete remain outside the journey. | Scope and section map are still automatic projections rather than persisted marketer decisions. Service Profile remains review-required; typed Ads/keyword evidence, real measurement and internal links are missing. Real generated text remains 5/10. | Persist reviewed service/intent/CTA and section-map decisions (`amj2.1`), then add typed demand evidence; separately replace historical `CODEX_API_KEY` connector readiness. |
