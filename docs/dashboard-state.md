...
`r564.6`, `r564.3`, `c9h9.4` and `/ahrefs` slice `3bst.7` are closed. The
current `/ahrefs` manual decision is above its cards; the next confirmed issue
there is API latency (`c9h9.17`), not another React matching rule. The deleted
legacy planner must not return.
...
| `/ahrefs` | 7/10 | SEO signal drilldown with an API-owned manual-only cross-check state. Above the card gallery, the first decision region now says that 6 topics require manual GSC/WordPress review, has zero exact confirmations and zero queue actions, and distinguishes `gotowe` data from a brief-ready decision. | `GET /api/ahrefs/diagnostics`, `getAhrefsDiagnostics()` and the shared `ContentAhrefsCandidateRow` schema; the route renders `gap_read_contract` directly. | Gap/authority signals, freshness, exact/weak/missing source lineage, evidence summary and one safe manual next step without raw IDs above details. Desktop/mobile proof is `.local-lab/proof/3bst7-ahrefs/`; mobile `scrollWidth=390`. | Ahrefs cannot drive publication or a content brief alone; the operator must still cross-check individual topics. Current endpoint latency is a real usability blocker: consecutive healthy HTTP reads took 14.654183 s and 15.872616 s. | Keep it as input to `/content-workflow`. `3bst.7` is closed; `c9h9.17` must measure and reduce existing diagnostics latency with relevant-evidence invalidation, without changing matching rules, adding an endpoint or creating a direct action. |
