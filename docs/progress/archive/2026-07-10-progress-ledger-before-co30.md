...
- Marketer-facing text must defend itself: every empty, missing or blocked
  ...
  absent.
- 2026-07-10 `wilq-seo-50wa.9` replaced the extracted-test dynamic bridge with
  five focused support leaves. `tests/test_api_contracts.py` dropped from 2611
  to 1857 physical lines; the full API-contract suite passed and no legacy
  bridge imports remain. Parent `wilq-seo-50wa` stays open for the remaining
  over-budget test bodies.
- 2026-07-10 `wilq-seo-b4kg.1` converted `wilq.schemas` into a compatible
  package and moved shared connector/evidence/metric contracts to
  `wilq/schemas/core.py`. The compatibility module fell from 4913 to 4484
  non-empty LOC while existing imports, OpenAPI and the full API-contract suite
  remained green. Parent `wilq-seo-b4kg` stays open for the next domain split.
- 2026-07-10 `wilq-seo-b4kg.2` moved ActionObject, knowledge-card/expert and
  marketing-brief/tactical-queue contracts into `wilq/schemas/actions.py`,
  `knowledge.py` and `marketing.py`. The compatibility facade fell further to
  3402 non-empty LOC; full API contracts, Ruff, mypy and live OpenAPI stayed
  green. Parent `wilq-seo-b4kg` remains open for Ads/Merchant/content/analytics.
- 2026-07-10 `wilq-seo-b4kg.3` moved Ads and Demand Gen contracts into
  `wilq/schemas/ads.py`. The compatibility facade is now 1539 non-empty LOC;
  Ads/action contracts, full API contracts, Ruff, mypy and live OpenAPI remain
  green. Parent `wilq-seo-b4kg` remains open for Merchant/content/analytics.
- 2026-07-10 `wilq-seo-b4kg.4` moved Merchant, Content, GA4, Localo, Ahrefs and
  Command Center/Daily Check contracts into explicit sibling modules. The
  public `wilq.schemas` facade is now 12 lines; full contracts, Ruff, mypy and
  live OpenAPI remain green. The schema-split parent is now closed; broader
  test/dashboard/Ads-diagnostics cleanup remains open.
- 2026-07-07 dashboard roast changed the `/content-workflow` direction from
...
- 2026-07-07 `/knowledge` now keeps the first screen decision-impact first:
  the preview shows one primary knowledge-backed decision, while additional
...
- 2026-07-04 skill coverage proof now has freshness semantics. A passing
  ...
  Wilku UAT.
- 2026-07-04 Ahrefs now has an API-owned review card on
  `operator_summary.review_*`, validates the safe
  `act_prepare_content_refresh_queue` handoff in smoke output and passes the
  fresh `wilq-ahrefs-gap-finder` eval at
  `.local-lab/evals/codex-skill/20260704T070718Z/wilq-ahrefs-gap-finder/result.json`
  with score `10/10`, one validated action candidate and no failure tags.
...
- 2026-07-04 Content/GSC freshness is now API-owned and visible on
  `/content-workflow`: `/api/content/diagnostics` exposes
...
- 2026-07-04 social history inventory now also exposes an API-owned
  `input_template` for collecting metadata-only LinkedIn/Facebook history:
  reviewer, collected date, required channel items and instruction excluding
  raw post bodies, comments, user data and tokens. `/social-publisher` renders
  the template summary, and the social skill smoke/eval verifies it.
...
- 2026-07-04 Goal 005 pre-demo audit now also reports ActionObject mutation
...
- 2026-07-04 historical live dashboard usefulness audit against
  `http://127.0.0.1:8000` passed with 15 surfaces, 13 `demo_ready`,
  2 `review_ready`, 0 blocked and high automated scores.
  Treat this as obsolete confidence history only; the current route-by-route
  dashboard truth lives in `docs/dashboard-state.md` and later UI roasts
  overrule this score.
...
- 2026-07-04 `scripts/wilq_stage_snapshot.py` now turns the nearest sub-10
...
- 2026-07-04 skill eval hardening now rejects future visible operator fields
...
- 2026-07-04 skill tuning packets now include a fillable reviewer scorecard:
...
- 2026-07-04 auditable reviewer scorecards now exist for the nearest sub-10
  skills. Ads Doctor rerun eval reached 10/10. GSC Content Doctor rerun stayed
  9/10, so its scorecard is now `popraw`; Ahrefs Gap Finder also stays
...
- 2026-07-04 Goal 005 completion guard has regression coverage for that UAT
  ...
  Goal 005 UAT result.
- 2026-07-04 Goal 005 owner-defer now must name private-source /
...
- 2026-07-04 Goal 005 Wilku session card and Service Profile handoff now render
...
- 2026-07-03 the Goal 005 session card now leads with the human content
  ...
  JSON proof value.
- 2026-07-03 the Goal 005 session card now labels review materials by their
...
- 2026-07-03 source fact coverage audit markdown now opens with a Polish
  readiness verdict instead of raw `source_backed_review_required`/`false`
- 2026-07-03 `/service-profile` dashboard now translates review scopes,
...
- `/command-center` dashboard first screen now renders API-owned top-level
  ...
  morning plan.
- Legacy content decision usefulness review had a short Wilku-facing decision
  card in the historical handoffs archive. Live
...
- Goal 002 marketer-facing content workflow route started under
  ...
  content, publish to WordPress or add behavior to `ContentWorkflowSurface`.
...
- `App.tsx` route composition uses a dedicated route renderer map. Focused
  ...
  `git diff --check` and browser proof for `/merchant`, `/content-workflow` and
...
- Content metric tiles no longer show a bare missing-state word when a metric
  ...
  `git diff --check` and browser proof for `/content-workflow` passed.
