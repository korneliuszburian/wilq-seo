...
def build_diagnostic_data_readiness(
    *,
    ...
    """Project confirmed facts or an explicit recovery state."""
...
def _missing_readiness(
    ...
    state, state_label, reason = _missing_state(connector, latest_refresh)
    return _nonfactual_readiness(
        connector=connector,
        latest_refresh=latest_refresh,
        evidence_ids=evidence_ids,
...
def _missing_state(
    ...
    if latest_refresh and latest_refresh.status == ConnectorRefreshStatus.blocked:
        return (
            ...
            (
                "WILQ nie ma potwierdzonych metryk, bo ostatni odczyt źródła "
                "został zablokowany."
            ),
        )
