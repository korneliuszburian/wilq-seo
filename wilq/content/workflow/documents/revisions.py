...
from wilq.content.workflow.documents.revision_document_validation import (
    validate_full_document as _validate_full_document,
)
...
def _validate_research_packet_binding(
    *,
    research_packet_id: str | None,
    research_packet_digest: str | None,
    document_kind: str | None = None,
) -> None:
    if (research_packet_id is None) != (research_packet_digest is None):
        raise ValueError("Research packet ID and digest must be supplied together.")
    if document_kind == "new_page" and research_packet_id is not None:
        raise ValueError("New-page revisions cannot carry a research packet.")


def _validate_revision_packet_metadata(
    *,
    research_packet_id: str | None,
    research_packet_digest: str | None,
    metadata: ContentDraftRevisionProposalMetadata | None,
    ...
    if metadata is None:
        return
    if (
        metadata.research_packet_id != research_packet_id
        or metadata.research_packet_digest != research_packet_digest
    ):
        raise ValueError("Draft revision packet binding must match its proposal metadata.")
