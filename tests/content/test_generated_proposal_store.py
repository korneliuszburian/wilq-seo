...
def test_inflight_generation_is_not_stale_before_deadline_grace(tmp_path: Path) -> None:
    store = ContentPlanningProposalStore(tmp_path / "state.sqlite3")
    response = ContentPlanningProposalResponse(
        status="generating",
        work_item_id="content_work_item_grace",
        service_card_id="ekologus_service_demo",
        safe_next_step="Plan jest przygotowywany.",
    )
    digest = "c" * 64
    ...
    with sqlite3.connect(store.path) as connection:
        connection.execute(
            "UPDATE content_planning_generation_jobs SET updated_at = ? WHERE work_item_id = ?",
            (
                (datetime.now(UTC) - timedelta(seconds=200)).isoformat(),
                response.work_item_id,
            ),
        )
