...
def _section(
    *, requirement_ids: list[str], evidence_ids: list[str]
) -> ContentPlanningModelSection:
...
def test_regulatory_requirements_need_a_planned_section_with_exact_evidence() -> None:
    ...
    complete = regulatory_planning_lineage_errors(
        ...
        _output(
            _section(
                requirement_ids=["regulated_scope"], evidence_ids=["ev_scope"]
            ),
            _section(
                requirement_ids=["regulated_deadline"], evidence_ids=["ev_deadline"]
            ),
        ),
    )
