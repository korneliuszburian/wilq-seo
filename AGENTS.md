...
| `/content-workflow` | Content Workflow usefulness scored: 7.5/10 as an honest end-to-end gate, 4.5/10 as production draft readiness and 8.5/10 for write/claim blocking. Dashboard now starts with `Workflow treści: co dziś zrobić` and shows that only one candidate passed gates while brief/draft/WordPress remain blocked. | Next tuning: connect the full Claim Ledger as a visible draft gate and run one approved-current knowledge item through preflight -> brief -> draft-only proof. |
