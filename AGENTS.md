...
2. `docs/current-cleanup-state.md` - current senior cleanup handoff; read before continuing repo cleanup, test splitting or dashboard/API refactors.
3. `docs/dashboard-state.md` - current per-route dashboard usefulness/API map; read before dashboard work.
4. `docs/PROGRESS.md` - latest short progress ledger and current gaps.
5. `docs/goals/archive/005-goal.md` - active goal and current execution boundaries.
6. `PLANS.md` - active long-running ExecPlan and latest decision log.
7. `docs/goals/001-goal.md` - historical cleanup contract and owner-deferred UAT context.
8. `docs/evals/skill-eval-ledger.md` - manual and non-interactive skill eval evidence.
9. `bd prime` - Beads recovery context, persistent task memories and session rules.
10. `bd ready --json` - current unblocked development work graph.
...
The current dashboard usefulness map lives in `docs/dashboard-state.md`.
Do not copy the per-route table into AGENTS.md. Before dashboard work, read
that file and update the affected row in place. Treat old live-audit snapshots,
`demo_ready` labels and eval scores as historical signals only, never as proof
that a marketer-facing screen is finished.

Current dashboard priority: `/content-workflow` is the primary `Treści i SEO`
workspace. Legacy planner routes and placeholder drilldowns must stay deleted,
hidden or technical unless a real API-owned workflow exists.
...
5 on a 1-10 usefulness scale: score 4 means follow-up is needed, score 3 is a
product gap, score 7 is a strong operator workflow and score 10 means
Wilku-ready / BDOS-class output that can be shown or used directly. Stale connector
